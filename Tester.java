...
class Tweet {
	...
	public String text = null; // e.g. @BossGotCash: Shoutout to the guy who got world record of 1 million
								// mileage on his Honda accord today.. Without changing engine, @AliMo103
	...
	/**
	 * parseTweet() is a static method that takes a String representation of a
	 * tweet. It then parses and extracts information from such a string, puts the
	 * information in a new Tweet object and returns it. If the input string is
	 * null, malformed, or does not contain all the tweet elements, it simply
	 * returns null. A valid input string has the following format:
	 * <tweetID>,<timestamp>,“[<uid>]<text>” Example of the input String:
	 * 128590282278187009,20111024,"[@ephan331are]@BossGotCash: Shoutout to the guy
	 * who got world record of 1 million mileage on his Honda accord today.. Without
	 * changing engine, @AliMo103"
	 */
	public static Tweet parseTweet(String str) {
		if (str == null)
			return null;
		String pStr = "([0-9]+),([0-9]+),\"\\[([@a-zA-Z0-9_]+)\\](.*)\""; // <--Implement your regular expression here
	...
	public String getCleanText() {
		String str;
		str = text.replaceAll("@(.*) ", "");
		str = ((str.replaceAll("#[0-9A-Za-z_]+ ", "")).replaceAll("http://[A-Za-z0-9-_./]+", ""));
		str = str.replaceAll("[^A-Za-z0-9_]+", " ").toLowerCase();
